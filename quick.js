...
function renderSchedule(mineData) {
  ...
  root.innerHTML = `<main class="quick-shell">${brand()}
    ...
    <div class="schedule-grid"><form id="response-form" class="quick-card"><h2>填寫我的時間</h2><p>先從月曆點選你要填寫的日期，再選早上、下午、晚上或 X。儲存後仍可隨時回來修改日期。</p>${periodLegendMarkup(periodRanges)}<label>玩家名稱<input name="playerName" maxlength="30" value="${escapeHtml(mineData?.playerName || localStorage.getItem("gather-party-player") || "")}" required></label><div class="date-picker-head"><h3 id="response-month-title">${responseMonthCursor.getFullYear()} 年 ${responseMonthCursor.getMonth() + 1} 月</h3><div class="date-picker-nav"><button class="mini-button" id="response-prev" type="button">‹</button><button class="mini-button" id="response-today" type="button">今</button><button class="mini-button" id="response-next" type="button">›</button></div></div><div id="response-calendar">${responseCalendarMarkup()}</div><section class="batch-choice-panel"><div><h3>批次設定時段</h3><p id="batch-choice-count">已選 ${choices.size} 天</p></div><div class="batch-choice-actions">${PERIOD_KEYS.map(period => `<button class="choice-button batch-choice-button" type="button" data-batch-choice="${period}">${period}</button>`).join("")}<button class="choice-button no batch-choice-button" type="button" data-batch-choice="X">X</button><button class="choice-button clear batch-choice-button" type="button" data-batch-choice="clear">清除時段</button></div></section><details class="choice-details" id="choice-details" ${choices.size <= 7 ? "open" : ""}><summary>逐日調整 <span id="choice-summary-count">${choices.size} 天</span></summary><div class="choice-list" id="response-choice-list">${responseChoiceListMarkup(periodRanges)}</div></details><label>備註<textarea name="note" maxlength="500" placeholder="例如：晚上九點後才有空、這天可能需要再確認">${escapeHtml(mineData?.note || "")}</textarea></label><div class="quick-form-actions"><span class="muted">至少選擇一個日期，且每個日期都要選時段或 X</span><button class="button" type="submit">${mineData?.submitted ? "儲存變更" : "儲存我的時間"}</button></div></form>
...
function updateChoiceControlState() {
  ...
  const batchCount = document.querySelector("#batch-choice-count");
  const summaryCount = document.querySelector("#choice-summary-count");
  if (batchCount) batchCount.textContent = count
    ? `已選 ${count} 天，可一次套用到全部日期`
    : "請先從月曆選擇日期";
  if (summaryCount) summaryCount.textContent = `${count} 天`;
...
function applyBatchChoice(value, ranges) {
  if (!choices.size) return toast("請先從月曆選擇日期。");
  for (const selected of choices.values()) {
    if (value === "clear") {
      selected.clear();
    } else if (value === "X") {
      selected.clear();
  ...
  markResponseDirty();
  toast(value === "clear" ? "已清除所有已選日期的時段" : `已將「${value}」套用到 ${choices.size} 天`);
}
